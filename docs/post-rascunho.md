# Quatro resultados negativos ao tentar medir mira humana em CS2
...
Não cheguei a um detector, e este texto não propõe um. Cheguei a quatro
resultados negativos, e acho que valem mais do que a baseline valeria, porque
três deles invalidam coisas que a literatura amadora de detecção de cheat
faz rotineiramente sem verificar.

Tudo é reprodutível: código aberto, e cada afirmação abaixo tem teste.

---

## O que funcionou, para calibrar o resto

Vale começar pelo que deu certo, senão o resto parece pessimismo.

O modelo de dois componentes do controle motor (Woodworth, 1899; lei de
Fitts) prevê que movimento humano dirigido a alvo tem uma fase balística
rápida que erra sistematicamente, seguida de correção sob feedback visual.
Isso implica três marcas mensuráveis. Nas 88 janelas de um demo:

| Predição | Observado |
|---|---|
| caminho percorrido > distância líquida | **100%** das janelas (mediana 1,35) |
| múltiplos submovimentos | ≥2 picos em **94,3%** |
| existe fase de acomodação | >0 em **98,9%** |

Cem por cento na primeira é o número que mais tranquiliza: `path_ratio < 1`
é geometricamente impossível, então se o unwrap de yaw ou a distância de
grande círculo estivessem errados, apareceriam violações. Não apareceu
nenhuma.

Há também estrutura não circular: movimentos grandes são proporcionalmente
mais retos (Spearman entre deslocamento e `path_ratio` = **−0,347**). Faz
sentido — a fase corretiva tem tamanho aproximadamente fixo, então pesa mais
numa correção curta.

Ou seja: o pipeline mede movimento motor real. É a partir daí que os
resultados negativos doem.
...
| Feature | 64 Hz | 32 Hz | razão |
|---|---|---|---|
| `snap_fraction` | 0,129 | 0,218 | **1,73x** |
| `n_peaks` | 7,0 | 4,0 | **0,40x** |
| `peak_speed_dps` | 91,0 | 76,1 | 0,85x |
| `path_ratio` | 1,355 | 1,339 | 0,995x |
| `net_disp_deg` | 10,14 | 10,10 | 1,00x |
...
## Resultado 4 — o tamanho do efeito é escolha do adversário

Este é o mais estrutural, e não some com mais dado.
...
testes, do mais forte ao mais circunstancial:
